...
const PostModal = ({ post, setPosts }: PostModalProps) => {
	...
	const formik = useFormik({
		initialValues: {
			...
			postEditorState: EditorState.createWithContent(convertFromRaw(JSON.parse(post.content))),
		},
						...
						<div className="modalActions mt-auto p-3 d-flex">
							...
							{isEditMode && (
								<div>
									...
									<button type="button" className="btn btn-secondary ml-3" onClick={resetForm}>
										Cancel
									</button>
								</div>
							)}
							<div className="ml-auto">
								<button
									...
									data-dismiss="modal"
									onClick={() => history.push(`/categories/${match.params.categoryId}`)}
								>
									Close
								</button>
							</div>

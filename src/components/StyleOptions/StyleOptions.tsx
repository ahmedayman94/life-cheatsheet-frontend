...
const StyleOptions = ({ postEditorState, onEditorChange }: StyleOptionsProps) => {
	...
	function onOlListClick() {
		const newState = RichUtils.toggleBlockType(postEditorState, "ordered-list-item");
		onEditorChange(newState);
	}

	function onUlListClick() {
		const newState = RichUtils.toggleBlockType(postEditorState, "unordered-list-item");
		onEditorChange(newState);
	}
